 ...
 *
 * mstring/generic_api/schedulers/sched_default.c: Data types and function prototypes
...
#define mstring_sched_type_t uint64_t
...
typedef struct __mstring_sched_prio_array {
  mstring_sched_type_t bitmap[EZA_SCHED_TOTAL_WIDTH];
  list_head_t queues[EZA_SCHED_TOTAL_PRIOS];
  ulong_t num_tasks;
} mstring_sched_prio_array_t;
...
typedef struct __eze_sched_taskdata {
  ...
  mstring_sched_prio_array_t *array;
} mstring_sched_taskdata_t;

#define is_rt_task(t) (((t)->sched_discipline == SCHED_RR) ||   \
                       ((t)->sched_discipline) == SCHED_FIFO)

typedef struct __mstring_sched_cpudata {
  spinlock_t lock;
  bound_spinlock_t __lock;
  scheduler_cpu_stats_t *stats;
  mstring_sched_prio_array_t *active_array,*expired_array;
  mstring_sched_prio_array_t arrays[EZA_SCHED_NUM_ARRAYS];
  task_t *running_task;
  cpu_id_t cpu_id;
} mstring_sched_cpudata_t;

extern mstring_sched_cpudata_t *sched_cpu_data[EZA_SCHED_CPUS];

/* Array must be locked prior to calling this function !
 */
static inline void __add_task_to_array(mstring_sched_prio_array_t *array,task_t *task)
{
  mstring_sched_taskdata_t *sched_data = (mstring_sched_taskdata_t *)task->sched_data;
 ...
 */
static inline void __remove_task_from_array(mstring_sched_prio_array_t *array,task_t *task)
{
  mstring_sched_taskdata_t *sched_data = (mstring_sched_taskdata_t *)task->sched_data;
  priority_t prio = task->priority;
...
static inline task_t *__get_most_prioritized_task(mstring_sched_cpudata_t *sched_data)
{
  mstring_sched_prio_array_t *array = sched_data->active_array;
  bit_idx_t idx = FIND_FIRST_BITMAP_BIT(array);

  if( idx != INVALID_BIT_INDEX ) {
    list_head_t *lh = &array->queues[idx];
    if( !list_is_empty(lh) ) {
      mstring_sched_taskdata_t *sdata = list_entry(list_node_first(lh),mstring_sched_taskdata_t,runlist);
      return sdata->task;
    } else {
...
#define EZA_TASK_SCHED_DATA(t) ((mstring_sched_taskdata_t *)t->sched_data)
...
static inline mstring_sched_cpudata_t *get_task_sched_data_locked(task_t *task,
                                                              long *is,bool cyclic)
{
  do {
    ulong_t cpu=task->cpu;
    mstring_sched_cpudata_t *cdata=sched_cpu_data[cpu];
